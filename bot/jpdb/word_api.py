...
class JpdbWordApi:
    def __init__(self):
        ...
        self.gembot = Gembot(
            0, GEMINI_UTILS_MODEL) if GEMINI_ENABLED else None
    ...
    def parse_alt_forms(self, first_result) -> list:
        alt_forms = []
        alt = first_result.find(
            'div',
            class_='subsection-other-spellings'
        )
        if alt:
            ...
            for alt in alt_spellings:
                ...
                if a_tag:
                    ...
                    kanji = ''.join(
                        ruby.get_text(strip=True) for ruby in ruby_tags
                    ) if ruby_tags else a_tag.get_text(strip=True)
