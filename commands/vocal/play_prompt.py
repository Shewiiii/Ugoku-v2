...
class PlayPrompt(commands.Cog):
    ...
    @commands.slash_command(
        name='play-prompt',
        description='Search a song from Spotify to play in vc.'
    )
    async def play_prompt(
        ...
        offset: discord.Option(
            int,
            description="Offset the search results.",
            default=0
        ),  # type: ignore
        ...
        playlist_match = re.findall(
            r"https?://open\.spotify\.com/(?:(?:intl-[a-z]{2})/)?"r"playlist/(?P<ID>[0-9a-zA-Z]{22})",
            query
        )
        ...
        if type == 'artist':
            ...
            track_infos = await spotify.get_tracks(id_=artist['id'], offset=offset, type='artist')
            cover = artist['images'][0]['url'] if artist.get(
                'images') else None
        else:
            if playlist_match:
                try:
                    ...
                    playlist_info = await asyncio.to_thread(
                        spotify.sessions.sp.playlist,
                        playlist_id
                    )
                    playlist_tracks = await asyncio.to_thread(
                        spotify.sessions.sp.playlist_tracks,
                        playlist_id=playlist_id, offset=offset
                    )
                    items = [
                        t['track'] for t in playlist_tracks['items']
                    ]
                    cover = playlist_info['images'][0]['url'] if playlist_info.get(
                        'images') else None
                    embed_title = playlist_info['name']
                except spotipy.exceptions.SpotifyException:
                    await ctx.respond("Content not found! Perhaps you are trying to play a private playlist?")
                    return
            else:
                items = await spotify.search(query, min(max_number_of_results, 25), offset, type=type)
                artist = await asyncio.to_thread(spotify.sessions.sp.artist, items[0]['artists'][0]['id'])
                cover = artist['images'][0]['url'] if artist.get(
                    'images') else None
        ...
        class SelectView(discord.ui.View):
            ...
            def update(self) -> None:
                ...
                end_index = min(
                    start_index + self.max_per_page, len(display_names))
                queue = '\n'.join(
                ...
                [search_embed.add_field(name='', value=part)
                 for part in splitted[1:]]

                # Buttons
                self.children[0].disabled = self.page <= 1
                self.children[1].disabled = len(
                    display_names) <= self.page * self.max_per_page
            ...
            async def play(self, interaction: discord.Interaction, track_infos: list[dict]) -> None:
                interaction_voice = interaction.user.voice
                if not interaction_voice or (interaction_voice and ctx.author.voice.channel != interaction_voice.channel):
                    return
                ...
                asyncio.create_task(session.add_to_queue(
                    ctx,
                    track_infos,
            ...
            @discord.ui.select(
                ...
                options=[discord.SelectOption(
                    label="1. You should not see this")]
            )
